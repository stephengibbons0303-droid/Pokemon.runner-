#!/usr/bin/env python3
"""
Extract Scizor PROJECTILE sprites — the attacks that fly across the duel toward
...
Saves one tightly-cropped PNG per move (sciz_proj_*.png), scaled to a uniform
...
SRC = os.path.join(ROOT, 'tools', 'scizor_sheets', 'projectiles.png')
# reading order TL, TR, BL, BR
ORDER = [('bp', 'Bullet Punch'), ('xs', 'X-Scissor'), ('sd', 'Swords Dance'), ('as', 'Air Slash')]
...
def main():
    ...
    for box, (key, label) in zip(find_frames(a), ORDER):
        ...
        outp = os.path.join(ROOT, f'sciz_proj_{key}.png')
        im.save(outp)
        print(f'{label:13s} -> sciz_proj_{key}.png  {im.width}x{im.height}')
        previews.append(im)
    ...
    sheet.convert('RGB').save('/tmp/sciz_proj_preview.png')
    print('preview -> /tmp/sciz_proj_preview.png')
